...
Note: No additional programs from the share library suite other than "shcoind" are required in order to run the coin+stratum service. The C share library can be staticly linked against the coin service, and a 'make install' in the share-coin build area is not required to run the program.
